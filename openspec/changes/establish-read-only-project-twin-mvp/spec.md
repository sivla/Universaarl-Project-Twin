...
Der Twin MUSS einen Repositoryalias und einen konfigurierbaren Freigabebranch verwenden. Der Freigabebranch kann später `main` oder `master` heißen und DARF weder in der fachlichen Oberfläche noch im Adapter fest verdrahtet werden. Bei einem Serverstart wird seine Spitze genau einmal auf eine vollständige Commit-SHA aufgelöst. Index, Katalog, Allowlist, Blobs, Modi, Digests und Referenzen MÜSSEN vollständig grün sein, bevor dieser Commit atomar zum aktiven Stand wird.

Akzeptanz: Ein fehlender oder ungültiger neuer Kandidat verändert den zuletzt gültigen commitgebundenen Stand nicht. Existiert noch kein gültiger Stand, antwortet die API weiterhin mit HTTP 503. Die Oberfläche zeigt neutral Freigabebranch, Aktualität und letzten erfolgreichen Aktualisierungszeitpunkt; SHA und Digest bleiben in nachgelagerten technischen Details. Entwicklungs- und Featurebranches werden niemals ohne ausdrücklichen Bootstrap-Integrationslauf sichtbar.
