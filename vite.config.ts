  ...
  return {
    plugins: [react(), {
      ...
      async configureServer(server) {
        ...
        const stableBranch = (process.env.UABC_STABLE_BRANCH || 'main').trim();
        ...
        let registry: ReturnType<typeof productionRegistry>;
        let channel: { branch: string; status: 'current' | 'stale'; lastValidatedAt: string; notice: string | null };
        try {
          const candidateCommit = git(['rev-parse', '--verify', `refs/heads/${stableBranch}^{commit}`]);
          const candidateTree = git(['rev-parse', '--verify', `${candidateCommit}^{tree}`]);
          registry = await validateRegistry(candidateCommit, candidateTree, stableBranch, true);
          channel = { branch: stableBranch, status: 'current', lastValidatedAt: new Date().toISOString(), notice: null };
        } catch {
          registry = await validateRegistry(officialBcBasicSnapshotAnchor.commit, officialBcBasicSnapshotAnchor.tree, officialBcBasicSnapshotAnchor.bootstrapBranch, false);
          channel = { branch: stableBranch, status: 'stale', lastValidatedAt: new Date().toISOString(), notice: 'Der konfigurierte Freigabebranch ist noch nicht gültig verfügbar. Der letzte vollständig validierte Stand bleibt sichtbar.' };
        }
        server.middlewares.use(async (req, res, next) => {
          try {
            ...
            if (result.status === 200 && pathname.endsWith('/state') && result.body && typeof result.body === 'object' && 'source' in result.body) {
              ...
              if (body.source) body.source.channel = channel;
            }
