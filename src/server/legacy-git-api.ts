/**
 * Ausschließlich nichtproduktive Regression für den entfernten Git-Laufzeitpfad.
 * Produktive Starter und Vite dürfen dieses Modul nicht importieren.
 */
...
export async function dispatchProjectApi(method: string, pathname: string, registry: readonly ProjectEntry[], readState: StateReader = createTwinState): Promise<ApiResult> {
  try {
    ...
    if (pathname === '/api/projects') {
      const projects = publicProjects(registry);
      return projects.length <= renderLimits.projects ? { status: 200, body: { projects } } : safeError('PROJEKTLISTE_ZU_GROSS', 503);
    }
    ...
    try {
      ...
      evidenceId = match[3] ? decodeURIComponent(match[3]) : '';
    ...
    if (match[2] === 'state') {
      try {
        return { status: 200, body: await readState(project.id, project.sourceRoot, { ...(project.sourceBinding ? { sourceBinding: project.sourceBinding } : {}), ...(project.sourceContract ? { projectDataContract: project.sourceContract } : {}) }) };
      } catch (error) {
        return project.sourceContract && error instanceof AdapterSourceError ? safeError('SNAPSHOT_VERTRAG_BLOCKIERT', 503) : safeError('QUELLE_NICHT_VERFUEGBAR', 503);
      }
    }
    if (match[4]) {
      let resourceId: string;
      try { resourceId = decodeURIComponent(match[4]); } catch { return safeError('ANFRAGE_UNGUELTIG', 400); }
      ...
      try {
        ...
        return error instanceof AdapterSourceError ? safeError('SNAPSHOT_VERTRAG_BLOCKIERT', 503) : safeError('API_NICHT_VERFUEGBAR', 500);
      }
    }
