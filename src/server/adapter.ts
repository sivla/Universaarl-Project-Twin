...
const producerTicketRecordSchema = z.object({
  ...
  estimateHours: z.number().nonnegative(), actualHours: z.number().nonnegative(), remainingHours: z.number().nonnegative(), hourlyRate: z.number().nonnegative().nullable().optional(), netAmount: z.number().nonnegative().optional(), acceptance: z.array(z.object({ criterion: z.string().min(1).max(2_000), fulfilled: z.boolean() }).strict()).default([]),
  ...
  worklogs: z.array(z.object({ id: technicalId, taskId: technicalId, date: sourceDateSchema, role: z.string().min(1).max(160), hours: z.number().nonnegative(), activity: z.string().min(1).max(8_000), phase: z.string().min(1).max(80), billable: z.boolean(), hourlyRate: z.number().nonnegative().optional(), netAmount: z.number().nonnegative().optional(), actorRef: technicalId, actorType: producerActorTypeSchema, actionRole: z.string().min(1).max(160) }).strict()).default([]),
...
const producerTicketCatalogSchema = z.object({
  schemaVersion: z.literal(1), projectId: z.literal('UABC-BC-BASIC-001'), classification: z.enum(['synthetic-only', 'current-pilot-planning']), sourceContract: z.literal('evidence/simulation/project-story.json'), generated: z.literal(true),
...
function producerPresentationProjection(index: ProjectDataIndex, reader: CommitBlobReader, sources: readonly IndexedProjectSource[], catalog: z.infer<typeof projectDocumentCatalogSchema>, story: NonNullable<ProjectState['story']>) {
  ...
  const tickets = canonicalRecords.map((record) => {
    ...
    return { ticketId: record.id, type: record.type, ...producerTicketTypePresentation[record.type], parentId: record.parent, projectStoryRole: 'customer-readable' as const,
      phaseId: record.phaseId, phaseRefs: record.phaseRefs ?? [], billingSource: record.billingSource, estimateHours: record.estimateHours, actualHours: record.actualHours, remainingHours: record.remainingHours, amountEur: storyTicket.netAmount ?? storyTicket.worklogs.reduce((sum, worklog) => sum + (worklog.cost ?? 0), 0), billable: record.billable,
...
function projectStoryProjection(reader: CommitBlobReader, source: IndexedProjectSource, sources: readonly IndexedProjectSource[]) {
  ...
  const tickets = raw.tickets.map((ticket) => {
    ...
    return { id: String(ticket.id), type: String(ticket.type ?? 'task'), status: String(ticket.status ?? 'unbekannt'), statusReason: storyText(ticket.statusReason), summary: storyText(ticket.summary) ?? acceptance[0]?.text ?? String(ticket.id), description: storyText(ticket.description), reporter: storyText(ticket.reporter), reporterRole: storyText(ticket.reporterRole), assignee: storyText(ticket.assignee), assigneeRole: storyText(ticket.assigneeRole), priority: storyText(ticket.priority), parent: typeof ticket.parent === 'string' ? ticket.parent : null, dependencies: storyStrings(ticket.dependencies), acceptanceCriteria: acceptance, statusHistory, comments, worklogs, evidenceRefs: storyStrings(ticket.evidenceRefs), documentRefs: storyStrings(ticket.pageRefs), deliverableRefs: storyStrings(ticket.deliverableRefs), decisionRefs: storyStrings(ticket.decisionRefs), pageRefs: storyStrings(ticket.pageRefs), createdAt: storyDate((ticket as RecordValue).createdAt), closedAt: storyDate((ticket as RecordValue).closedAt),
      ...
      estimateHours: typeof ticket.estimateHours === 'number' ? ticket.estimateHours : null, actualHours: typeof ticket.actualHours === 'number' ? ticket.actualHours : null, remainingHours: typeof ticket.remainingHours === 'number' ? ticket.remainingHours : null, netAmount: typeof ticket.netAmount === 'number' ? ticket.netAmount : null, billable: typeof ticket.billable === 'boolean' ? ticket.billable : null };
  });
...
const spectra09ConformanceSchema = z.object({
  ...
  counts: z.object({ offerVersions: z.number().int(), pages: z.number().int(), tickets: z.number().int(), comments: z.number().int(), worklogs: z.number().int(), hours: z.number().optional(), cost: z.number().optional(), actualHours: z.number().optional(), actualNetAmount: z.number().optional(), timelineEvents: z.number().int(), hypercareDays: z.number().int().optional(), hypercareRecords: z.number().int().optional(), nativeRelations: z.number().int() }).passthrough().superRefine((counts, context) => {
    if ((counts.actualHours ?? counts.hours) === undefined || (counts.actualNetAmount ?? counts.cost) === undefined) context.addIssue({ code: z.ZodIssueCode.custom, message: 'Aktuelle Stunden und Kosten fehlen.' });
  }),
}).passthrough();

const v1DemoReadinessSchema = z.object({
  schemaVersion: z.literal(1), classification: z.enum(['synthetic-only', 'historical-reference-simulation']), currentAuthority: z.boolean().optional(), supersededBy: z.string().refine(validateContractPath).optional(), status: z.enum(['synthetisch-abgeschlossen', 'synthetic-closed-superseded']), productResult: z.literal('V1_STANDARDPRODUCT_READY'), result: z.literal('GO_SIMULATION'),
...
export function validateV1DemoReadiness(input: unknown): SpectraSummary {
  ...
  if (value.classification === 'historical-reference-simulation') {
    if (value.currentAuthority !== false || value.supersededBy !== 'evidence/simulation/project-story.json') sourceError('Die historische Referenzsimulation ist nicht eindeutig als nachrangig gebunden.');
    return {
      title: 'Historische Referenzsimulation', status: 'historical-reference-only',
      rationale: 'Dieser Abschlussstand ist ausschließlich historische Provenienz und trägt nicht zum aktuellen Projekt-, Budget- oder Arbeitsrollup bei.',
      activity: ['Aktuelle Autorität: nein', `Aktueller Projektstand: ${value.supersededBy}`, 'Keine historischen Freigaben, Worklogs oder Abschlusswerte im aktuellen Rollup'],
    };
  }
  return {
...
export function validateSpectra09ContractData(input: { release: unknown; conformance: unknown; reconciliation: unknown; provenance: unknown; exportMap: unknown; indexArtifacts: unknown; indexHash: string; projectionHash: string; catalogExtendedIndex?: boolean }) {
  ...
  const conformanceActualHours = conformance.counts.actualHours ?? conformance.counts.hours!;
  const conformanceActualCost = conformance.counts.actualNetAmount ?? conformance.counts.cost!;
  ...
  if (conformanceActualHours !== reconciliation.actual.hours || conformanceActualCost !== reconciliation.actual.amount) sourceError('Der aktuelle Stunden- oder Kostenstand widerspricht dem Spectra-Projektabgleich.');
...
function indexedArtifacts(index: ProjectDataIndex, reader: CommitBlobReader, sources: readonly IndexedProjectSource[]) {
  ...
  for (const source of sources) {
    ...
    if (declaration.kindId === 'confluence-page' || declaration.kindId === 'meeting-transcript') {
      ...
      const classification = typeof data.classification === 'string' && data.classification.trim() ? data.classification : null;
      const currentAuthority = typeof data.currentAuthority === 'boolean' ? data.currentAuthority : null;
      const currentRollupContribution = typeof data.currentRollupContribution === 'boolean' ? data.currentRollupContribution : null;
      if (declaration.kindId === 'meeting-transcript' && classification === 'historical-reference-simulation' && (currentAuthority !== false || currentRollupContribution !== false || data.supersededBy !== 'evidence/simulation/project-story.json')) sourceError('Ein historisches Besprechungsprotokoll ist nicht eindeutig aus dem aktuellen Rollup ausgeschlossen.');
      add({ id, kind: 'document', title: title ?? null, status, phase: null, wave: null, workstream: null, rationale: null,
        ...
        evidence: [], sourceType: declaration.kindId, documentType: declaration.kindId, meetingDate, classification, currentAuthority, currentRollupContribution, sourcePath: declaration.path });
      continue;
    }
