...
import type { Artifact, PresentationContract, ProjectDocument, ProjectState } from '../model';
...
function useSpaceSession(state: ProjectState, contract: PresentationContract) {
  const sourceVersion = state.source.catalog?.releaseId ?? state.source.commit ?? state.source.readAt;
  const key = `twin-spaces:${state.source.projectId}:${sourceVersion}`; const defaults = useMemo(() => defaultExpansion(contract), [contract]); const [expanded, setExpanded] = useState(defaults);
...
export function DocumentationSpaces({ state, open }: { state: ProjectState; open: (artifact: Artifact) => void }) {
  ...
  const ticketArtifacts = new Map(state.artifacts.filter((artifact) => state.story?.tickets.some((ticket) => ticket.id === artifact.id) || contract.jira.tickets.some((ticket) => ticket.ticketId === artifact.id)).map((artifact) => [artifact.id, artifact]));
