...
function TicketCard({ ticket, presentation, view, open, artifact, phaseLabel }: { ticket: StoryTicket; presentation: PresentationTicket; view: JiraView; open: OpenArtifact; artifact?: Artifact; phaseLabel: string }) {
  ...
  return <button className="ticket-card" onClick={() => artifact && open(artifact)} disabled={!artifact}>
    ...
    <span className="ticket-card-meta">
      ...
      {presentation.type !== 'task' && <span>Schätzung {presentation.estimateHours} Std. · Task-Rollup {presentation.actualHours} Std. Ist / {presentation.remainingHours} Std. Rest / {presentation.amountEur.toLocaleString('de-DE')} EUR</span>}
    </span>
  </button>;
}

function GroupDisclosure({ label, expanded, onClick, onOpen, presentation, prefix, rollup }: { label: string; expanded: boolean; onClick: () => void; onOpen?: () => void; presentation?: PresentationTicket; prefix: 'Phase' | 'Epic'; rollup?: { estimateHours: number; actualHours: number; remainingHours: number; amountEur: number } }) {
  return <div className="group-disclosure-row"><button className={`group-disclosure group-disclosure-${prefix.toLowerCase()}`} aria-expanded={expanded} aria-label={`${expanded ? 'Einklappen' : 'Aufklappen'}: ${prefix} ${label}`} onClick={onClick}><span aria-hidden="true">{expanded ? '⌄' : '›'}</span>{presentation && <TicketTypeIcon ticket={presentation} />}<strong>{prefix}: {label}</strong>{rollup && <span>{rollup.estimateHours} Std. geplant · {rollup.actualHours} Std. Ist · {rollup.remainingHours} Std. Rest · {rollup.amountEur.toLocaleString('de-DE')} EUR</span>}<small>{expanded ? 'Eingeblendet' : 'Ausgeblendet'}</small></button>{onOpen && <button className="group-detail-button" onClick={onOpen} aria-label={`${prefix}-Ticket ${presentation?.ticketId ?? label} im Detail öffnen`}>Details</button>}</div>;
}
...
export function TicketsPage({ state, open }: { state: ProjectState; open: OpenArtifact }) {
  ...
  const taskBasis = contract.jira.tickets.filter((ticket) => ticket.type === 'task' && ticket.billable).reduce((sum, ticket) => ({ hours: sum.hours + ticket.actualHours, amountEur: sum.amountEur + ticket.amountEur }), { hours: 0, amountEur: 0 });

  return <section className="tickets-page">
    <header><div><p>KANONISCHE PROJEKTSTORY</p><h2>Kanonischer Projekt-Backlog</h2><p>Board, Liste und Summen verwenden ausschließlich {contract.jira.canonicalTicketCount} producerdefinierte kundenlesbare Tickets.</p></div><span>Taskbasis: {taskBasis.hours} Std. · {taskBasis.amountEur.toLocaleString('de-DE')} EUR · keine historischen Traceability-Issues</span></header>
