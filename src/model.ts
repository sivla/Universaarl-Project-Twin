...
export const artifactSchema = z.object({
  ...
  classification: z.string().min(1).max(120).nullable().default(null), currentAuthority: z.boolean().nullable().default(null), currentRollupContribution: z.boolean().nullable().default(null),
...
export const setupWaveProjectionSchema = z.object({
  ...
  configurationState: z.object({
    baselineKind: z.literal('standard-cronus-demo'), baselineProvenance: z.literal('microsoft-standard-cronus-demo-data'), customerTargetRealized: z.literal(false), originMechanismStatus: z.literal('unbekannt-bis-wave0-readback'), copyRenameHypothesis: z.literal('nutzerhinweis-unbestaetigt'), setupStatus: z.literal('blockiert-bis-dom-readback-und-zielkonfiguration'), pilotConfigured: z.literal(false), writesApplied: z.literal(false), readbackStatus: z.literal('pending'),
    technicalCompanyName: sourceTechnicalIdSchema, internalCompanyId: z.null(), observedDisplayName: z.string().min(1).max(200), targetDisplayName: z.string().min(1).max(200),
  ...
  readinessPath: z.object({
    sourcePath: sourceRelativePathSchema,
    baselineKind: z.literal('standard-cronus-demo'),
    targetGate: sourceTechnicalIdSchema,
    targetStatus: z.string().min(1).max(120),
    readyToProdClaimed: z.literal(false),
    productionStartClaimed: z.literal(false),
    customerAcceptanceClaimed: z.literal(false),
    pilotConfigured: z.literal(false),
    writesApplied: z.literal(false),
    currentStageId: sourceTechnicalIdSchema,
    nextExecutableStep: z.string().min(1).max(200),
    stages: z.array(z.object({
      stageId: sourceTechnicalIdSchema,
      order: z.number().int().nonnegative(),
      name: z.string().min(1).max(300),
      status: z.string().min(1).max(120),
      dependencyStageIds: z.array(sourceTechnicalIdSchema).max(20),
      ticketRefs: z.array(sourceTechnicalIdSchema).min(1).max(50),
      bcWriteRequired: z.boolean(),
      writeAuthorized: z.literal(false),
      completed: z.literal(false),
      entryCriteriaCount: z.number().int().nonnegative(),
      plannedActionCount: z.number().int().nonnegative(),
      exitCriteriaCount: z.number().int().nonnegative(),
      evidenceTargetCount: z.number().int().nonnegative(),
      executionEvidenceCount: z.literal(0),
      stopOrRollbackDefined: z.literal(true),
    }).strict()).min(1).max(30),
  }).strict().optional(),
