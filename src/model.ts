...
export const artifactKinds = ['epic', 'story', 'task', 'change', 'capability', 'architecture', 'document', 'evidence'] as const;

export const artifactSchema = z.object({
  id: z.string().min(1), kind: z.enum(artifactKinds), title: z.string().min(1), status: z.string().min(1), phase: z.enum(phases),
  wave: z.string().default(''), workstream: z.string().min(1), rationale: z.string().default(''), parentId: z.string().nullable().default(null),
...
export const projectStateSchema = z.object({
  source: z.object({ branch: z.string().min(1), commit: z.string().regex(/^[a-f0-9]{40}$/), dirty: z.boolean(), readAt: z.string().datetime() }),
