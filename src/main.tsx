...
function SetupWaveOverview({ state }: { state: ProjectState }) {
  ...
  const statusLabel = (value: string) => ({ 'prepared-for-controlled-live-run': 'Für einen kontrollierten Lauf vorbereitet', 'prepared-not-executed': 'Vorbereitet, nicht ausgeführt', 'designed-not-executed': 'Entworfen, nicht ausgeführt', 'beobachtet-nur-lesend': 'Nur-lesend beobachtet', 'required-not-executed': 'Erforderlich, noch nicht ausgeführt', 'blocked-before-dom-readback': 'Vor DOM, Screenshot und BC-Feldreadback blockiert', 'missing-blocker': 'Fehlt und blockiert weitere Schritte', 'pending-wave-0-evidence': 'Zielstrategie offen', 'pending-resetpoint-evidence': 'Resetentscheidung offen', 'W0-01-read-company-identity': 'W0-01 · Firmenidentität nur-lesend prüfen', pending: 'Ausstehend' }[value] ?? value);
