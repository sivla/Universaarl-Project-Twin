...
function App() {
  ...
  return <div className="app-shell">
    ...
    {visibleSelection && <Detail artifact={visibleSelection} artifacts={visibleState?.artifacts ?? []} storyTicket={state?.value.story?.tickets.find((ticket) => ticket.id === visibleSelection.id)} presentationTicket={state?.value.presentation?.jira.tickets.find((ticket) => ticket.ticketId === visibleSelection.id)} select={(artifact) => setSelected({ viewKey, artifact })} close={() => setSelected(undefined)} />}
  </div>;
}
...
const businessFocus = [
  { title: 'Was ist verkauft?', question: 'Angebot, Scope, Nichtleistungen und Annahmen', pattern: /offer|proposal|requirements|project-reconciliation|project-story-offer/i },
  { title: 'Wo stehen wir?', question: 'Phasen, Fortschritt, Verantwortliche und Reifestufe', pattern: /project-plan|delivery-plan|phase-|project-story-timeline/i },
  { title: 'Was ist fertig?', question: 'Prozesse, Entscheidungen, UAT, Cutover und Hypercare', pattern: /process|playthrough|decision|uat|training|cutover|hypercare|completion/i },
  { title: 'Was ist offen?', question: 'Ausdrücklich offene Gaps, Risiken, Tickets und Kundenentscheidungen', pattern: /risk|gap|decision|readiness|jira|project-story-ticket/i },
  { title: 'Was ist als Nächstes zu tun?', question: 'Nächste belegte Handlung für Kunde und Projektteam', pattern: /handover|readiness|customer|decision|training|project-story-timeline/i },
] as const;
...
function BusinessOverview({ state, open }: { state: ProjectState; open: (artifact: Artifact) => void }) {
  ...
  const v1Acceptance = state.artifacts.find((artifact) => artifact.sourceType === 'synthetic-demo-evidence');
  const v1Ready = v1Acceptance?.activity.includes('V1_STANDARDPRODUCT_READY') === true;
  const reusableBacklog = state.artifacts.filter((artifact) => isWorkArtifact(artifact) && artifact.sourcePath === 'atlassian/jira/issues/bc-basic-project.yaml' && artifact.status?.toLowerCase() === 'backlog');
  ...
  return <section className="business-overview"><header><div><p>FACHLICHER PROJEKTÜBERBLICK</p><h2>Was jetzt für das Projekt wichtig ist</h2></div><span>Sandboxsimulation · keine reale BC-, Steuer- oder Kundenfreigabe</span></header><div className="business-focus-grid">{businessFocus.map((focus) => {
    ...
    const thematicMatches = state.artifacts.filter((artifact) => focus.pattern.test(`${artifact.sourceType ?? ''} ${artifact.documentType ?? ''} ${artifact.workstream ?? ''} ${artifact.title}`));
    const matches = focus.title === 'Was ist offen?' ? state.artifacts.filter((artifact) => isWorkArtifact(artifact) && openStatuses.includes((artifact.status ?? '').toLowerCase()) && !(v1Ready && artifact.sourcePath === 'atlassian/jira/issues/bc-basic-project.yaml' && artifact.status?.toLowerCase() === 'backlog')) : thematicMatches;
    const openItems = matches.filter((artifact) => openStatuses.includes((artifact.status ?? '').toLowerCase())).length;
    ...
    const startklarFacts = !startklar ? [] : focus.title === 'Was ist verkauft?' ? [startklar.offer, startklar.baseline]
      : focus.title === 'Wo stehen wir?' ? [`Startklar-Paket: ${startklar.workshops} Workshops · ${startklar.decisions} Entscheidungen · ${startklar.dataDeliveries} Datenlieferungen`]
        : focus.title === 'Was ist fertig?' ? [`Entry-Gate für Setup und UAT belegt · ${startklar.learningPaths} Rollenpfade mit positivem Fall, Fehler, Retest und Kompetenzpass`, operatorSummary ? `${operatorSummary} · keine künstlichen Trainings für P-001 oder P-016` : 'Operatorroutinen nicht belegt', 'Kompetenzpass ist simuliert und keine reale Kundensandboxfreigabe.', ...(v1Acceptance?.activity.slice(0, 4) ?? [])]
          : focus.title === 'Was ist offen?' ? [startklar.minimalPreparation ? 'Minimale Kundenvorbereitung ist beschrieben; technische Dokumentstatus zählen nicht als Kundenaufgabe.' : 'Kundenvorbereitung nicht belegt.', ...(v1Ready ? [`Keine offenen V1-Simulationsaufgaben oder P1/P2. ${reusableBacklog.length} Backlog-Einträge sind wiederverwendbare Planung und keine aktuelle Kundenhandlung.`] : [])]
            : [`Nächste Handlung: Kunde und Key User führen den Operator-Smoke-Test entlang der ${startklar.learningPaths} Rollenpfade aus; Support übernimmt nur das vollständige Diagnosepaket.`, escalationSummary ?? 'Eskalationsausgänge nicht belegt', ...operatorHandover, ...(v1Acceptance?.activity.slice(4) ?? [])];
...
function Current({ state, context, open }: { state: ProjectState; context: ProjectContext; open: (artifact: Artifact) => void }) {
  ...
  return <>
    <section className="snapshot-intro"><div><p>PROJEKTCOCKPIT</p><h2>{context.projectName}</h2><p>Repositorybasierte Sandboxsimulation der vollständigen Einführungsstory. Sie zeigt keine reale BC-Buchung, Steuer- oder Rechtsfreigabe und keine produktive Kundenaktivität.</p>{state.source.channel?.notice && <p className="honest-note">{state.source.channel.notice}</p>}</div><dl><dt>Datenstand</dt><dd>{state.source.channel?.status === 'stale' ? 'Veraltet · letzter gültiger Stand' : 'Aktuell validiert'}</dd><dt>Freigabekanal</dt><dd>{state.source.channel ? `Freigabebranch ${state.source.channel.branch}` : 'Commitgebundener Bootstrapstand'}</dd><dt>Simulationsstatus</dt><dd>{state.story?.status ? displayStatus(state.story.status) : 'Commitgebunden geprüft'}</dd><dt>Arbeitsnachweis</dt><dd>{state.story?.controls?.worklogHours ?? 0} Std. · {numberFormatter.format(state.story?.controls?.worklogCost ?? 0)} EUR</dd><dt>Offene P1/P2</dt><dd>{state.story?.controls?.openP1 ?? 0}/{state.story?.controls?.openP2 ?? 0}</dd></dl></section>
    <BusinessOverview state={state} open={open} />
...
function Detail({ artifact, artifacts, storyTicket, presentationTicket, select, close }: { artifact: Artifact; artifacts: Artifact[]; storyTicket?: StoryTicket; presentationTicket?: PresentationTicket; select: (artifact: Artifact) => void; close: () => void }) {
  ...
  return <div className="drawer-scrim"><div className="drawer" ref={ref} role="dialog" aria-modal="true" aria-labelledby="detail-title"><button className="drawer-close" onClick={close}>Schließen</button>{presentationTicket && <TicketTypeIcon ticket={presentationTicket} />}<h2 id="detail-title">{artifact.id}</h2><h3>{sourceText(artifact.title)}</h3><dl>
    <dt>Typ</dt><dd>{presentationTicket?.typeLabel ?? displayArtifactType(artifact.sourceType)}</dd><dt>Status</dt><dd>{displayStatus(artifact.status)}</dd><dt>Phase</dt><dd><code>{sourcePhaseLabel(artifact)}</code></dd>
    {storyTicket ? <><dt>Übergeordnetes Ticket</dt><dd>{storyTicket.parent ?? 'Keines'}</dd><dt>Priorität</dt><dd>{sourceText(storyTicket.priority)}</dd><dt>Verantwortung</dt><dd>{sourceText(storyTicket.assignee)}</dd><dt>Schätzung</dt><dd>{storyTicket.estimateHours === null ? 'Nicht belegt' : `${numberFormatter.format(storyTicket.estimateHours)} Std.`}</dd><dt>Ist</dt><dd>{sourceActualLabel(artifact)}</dd><dt>Rest</dt><dd>{storyTicket.remainingHours === null ? 'Nicht belegt' : `${numberFormatter.format(storyTicket.remainingHours)} Std.`}</dd><dt>Abrechenbar</dt><dd>{storyTicket.billable === null ? 'Nicht belegt' : storyTicket.billable ? 'Ja, ausschließlich als Aufgabe' : 'Nein, nur Task-Rollup'}</dd></> : <><dt>Arbeitsstrom</dt><dd>{sourceText(artifact.workstream)}</dd><dt>Planaufwand</dt><dd>{sourceEffortLabel(artifact)}</dd><dt>Erfasster Aufwand</dt><dd>{sourceActualLabel(artifact)}</dd><dt>Beginn</dt><dd>{sourceDateLabel(artifact.startDate)}</dd><dt>Fällig</dt><dd>{sourceDateLabel(artifact.dueDate)}</dd><dt>Abrechenbar</dt><dd>{artifact.billable === null ? 'Nicht belegt' : artifact.billable ? 'Ja' : 'Nein'}</dd><dt>Abrechnungswoche</dt><dd>{artifact.billingWeek ?? 'Nicht belegt'}</dd><dt>Abrechnungsstatus</dt><dd>{displayBillingStatus(artifact.billingStatus)}</dd><dt>Dokumenttyp</dt><dd>{displayDocumentType(artifact.documentType)}</dd><dt>Besprechungsdatum</dt><dd>{sourceDateLabel(artifact.meetingDate)}</dd><dt>Verantwortung</dt><dd>{sourceText(artifact.owner)}</dd><dt>Priorität</dt><dd>{sourceText(artifact.priority)}</dd></>}
    <dt>Quelle</dt><dd><code>{artifact.sourcePath}</code></dd>
  </dl><h4>{storyTicket ? 'Beschreibung' : 'Begründung und Inhalt'}</h4><p>{artifact.rationale || 'Nicht in der Quelle erfasst.'}</p>{storyTicket && <><h4>Akzeptanzkriterien</h4>{storyTicket.acceptanceCriteria.length ? <ul>{storyTicket.acceptanceCriteria.map((criterion) => <li key={criterion.text}>{criterion.fulfilled === null ? 'Nicht bewertet' : criterion.fulfilled ? 'Erfüllt' : 'Nicht erfüllt'}: {criterion.text}</li>)}</ul> : <p className="honest-note">Keine Akzeptanzkriterien belegt.</p>}<h4>Typisierte Kommentare</h4>{storyTicket.comments.length ? <ul>{storyTicket.comments.map((comment) => <li key={comment.id}><strong>{comment.type}</strong> · {comment.time ?? 'Datum nicht belegt'} · {comment.role ?? 'Rolle nicht belegt'}: {comment.text}{comment.evidenceRef && <>{' · '}<LinkedReference value={comment.evidenceRef} {...linkedReferences} /></>}</li>)}</ul> : <p className="honest-note">Keine Kommentare belegt.</p>}<h4>Einzelne Worklogs</h4>{storyTicket.worklogs.length ? <ul>{storyTicket.worklogs.map((worklog, index) => <li key={`${worklog.date}-${index}`}>{worklog.date ?? 'Datum nicht belegt'} · {worklog.role ?? 'Rolle nicht belegt'} · {worklog.hours} Std. · {worklog.activity ?? 'Aktivität nicht belegt'} · {worklog.phase ?? 'Phase nicht belegt'}</li>)}</ul> : <p className="honest-note">Keine Worklogs belegt.</p>}<h4>Statushistorie</h4>{storyTicket.statusHistory.length ? <ol>{storyTicket.statusHistory.map((item, index) => <li key={`${item.time}-${index}`}><time dateTime={item.time}>{sourceDateLabel(item.time)}</time> · {displayStatus(item.status)}</li>)}</ol> : <p className="honest-note">Keine Statushistorie belegt.</p>}{storyEvidence && <DetailReferences title="Positivgelistete Evidence" values={storyEvidence} {...linkedReferences} />}</>}{artifact.activity.length > 0 && !storyTicket && <><h4>Kommentare und Arbeitsnachweise</h4><ul>{artifact.activity.map((item, index) => <li key={`${index}-${item}`}>{item}</li>)}</ul></>}
...
function LinkedReference({ value, artifactById, select }: { value: string; artifactById: ReadonlyMap<string, Artifact>; select: (artifact: Artifact) => void }) {
  ...
  return target ? <button className="detail-reference-button" onClick={() => select(target)}><code>{value}</code> · öffnen</button> : <code>{value}</code>;
}
