...
function SetupWaveOverview({ state }: { state: ProjectState }) {
  ...
  const statusLabel = (value: string) => ({ 'prepared-for-controlled-live-run': 'Für einen kontrollierten Lauf vorbereitet', 'prepared-not-executed': 'Vorbereitet, nicht ausgeführt', 'designed-not-executed': 'Entworfen, nicht ausgeführt', 'beobachtet-nur-lesend': 'Nur-lesend beobachtet', 'required-not-executed': 'Erforderlich, noch nicht ausgeführt', 'missing-blocker': 'Fehlt und blockiert weitere Schritte', pending: 'Ausstehend' }[value] ?? value);
