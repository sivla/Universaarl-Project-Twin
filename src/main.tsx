...
function App() {
  ...
  return <div className="app-shell">
    ...
    {visibleSelection && <Detail artifact={visibleSelection} close={() => setSelected(undefined)} />}
  </div>;
}
...
function Current({ state, context, open }: { state: ProjectState; context: ProjectContext; open: (artifact: Artifact) => void }) {
  ...
  const storyGroups = [
    ['Angebot', story.filter((artifact) => artifact.sourceType === 'project-story-offer')],
    ['Seitenbaum', story.filter((artifact) => artifact.sourceType === 'project-story-page')],
    ['Tickets', story.filter((artifact) => artifact.sourceType?.startsWith('project-story-') && artifact.kind !== 'document')],
    ['Timeline', story.filter((artifact) => artifact.sourceType === 'project-story-timeline')],
    ['Hypercare', story.filter((artifact) => artifact.sourceType === 'project-story-hypercare')],
  ] as const;
  return <>
    ...
    {story.length > 0 && <section className="records"><header><div><p>COMMITGEBUNDENE PROJEKTSTORY</p><h2>Angebot bis Handover</h2></div><span>{story.length} Storyeinträge</span></header><p className="honest-note">Angebot, Seitenbaum, Tickets, Timeline und Hypercare werden ausschließlich aus dem positivgelisteten Storyvertrag des gebundenen Commits angezeigt.</p><div className="summary-grid">{storyGroups.map(([title, items]) => <button key={title} className="story-summary-card" onClick={() => items[0] && open(items[0])}><b>{items.length}</b><span>{title}</span>{items[0] && <small>{sourceText(items[0].title)}</small>}</button>)}</div></section>}
...
function Detail({ artifact, close }: { artifact: Artifact; close: () => void }) {
  ...
  </dl><h4>Begründung und Inhalt</h4><p>{artifact.rationale || 'Nicht in der Quelle erfasst.'}</p>{artifact.activity.length > 0 && <><h4>Kommentare und Arbeitsnachweise</h4><ul>{artifact.activity.map((item, index) => <li key={`${index}-${item}`}>{item}</li>)}</ul></>}
