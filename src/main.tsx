...
function Delivery({ state, open }: { state: ProjectState; open: (artifact: Artifact) => void }) {
  ...
  const executionEvidence = boundedList(state.artifacts.filter((artifact) => artifact.sourceType === 'country-company-information-execution-evidence'), renderLimits.artifacts);
...
function SetupWaveOverview({ state }: { state: ProjectState }) {
  ...
  const statusLabel = (value: string) => ({ 'prepared-for-controlled-live-run': 'Für einen kontrollierten Lauf vorbereitet', 'prepared-not-executed': 'Vorbereitet, nicht ausgeführt', 'designed-not-executed': 'Entworfen, nicht ausgeführt', 'beobachtet-nur-lesend': 'Nur-lesend beobachtet', 'missing-blocker': 'Fehlt und blockiert weitere Schritte' }[value] ?? value);
  return <section className="area-view setup-wave-overview"><header><div><p>VORBEREITUNGSSTAND</p><h2>Business Central Einrichtung</h2></div><span>Nur-lesend geprüft · keine Live-Ausführung</span></header>
    <div className="management-grid">
      <article><h3>Zielumgebung</h3><dl><dt>Umgebung und Mandant</dt><dd>{setup.target.environment} · <code>{setup.target.companyId}</code></dd><dt>Anwendung</dt><dd>{setup.target.application}</dd><dt>Plattform</dt><dd>{setup.target.platform}</dd><dt>Pilot</dt><dd>{setup.target.pilotName}</dd><dt>Legacy</dt><dd>{setup.target.legacyName}</dd></dl></article>
      <article><h3>Read-only Preflight</h3><dl><dt>Status</dt><dd>{statusLabel(setup.preflight.status)}</dd><dt>Arbeitsdatum</dt><dd>{sourceDateLabel(setup.preflight.workingDate)}</dd><dt>Beobachtete Rolle</dt><dd>{setup.preflight.operator.permissionSet} · {setup.preflight.operator.userId}</dd><dt>Gebietsschema</dt><dd>{setup.preflight.locale}</dd></dl></article>
      ...
      <article><h3>Schreibsperre und nächster Schritt</h3><dl><dt>Schreibzugriff</dt><dd>Nicht autorisiert</dd><dt>Resetpunkt</dt><dd>{statusLabel(setup.preflight.resetPoint.status)}</dd><dt>Nächster zulässiger Schritt</dt><dd>{setup.writeGate.nextAllowedStep}</dd></dl><p className="honest-note">Die Projektion beschreibt ausschließlich den Soll- und Vorbereitungsstand. Sie behauptet keine Umbenennung oder Ausführung in einer realen BC-Instanz.</p></article>
    </div>
  </section>;
}
