...
describe('Normalisierungsvertrag', () => {
  ...
  it('liefert vollständige Listen und setzt Pagination nach einem Filter zurück', () => { const values = Array.from({ length: 57 }, (_, index) => `ID-${index + 1}`); const result = boundedList(values, 24); expect(result.items).toHaveLength(57); expect(result.items).toEqual(values); expect(result.visible).toBe(57); expect(result.total).toBe(57); expect(result.limited).toBe(false); expect(result.disclosureThreshold).toBe(24); expect(new Set(result.items).size).toBe(57); const filtered = values.filter((value) => value.endsWith('1') || value.endsWith('2')); const filteredPage = paginateList(filtered, 3, 10); expect(filteredPage.page).toBe(2); expect(filteredPage.total).toBe(filtered.length); const resetPage = paginateList(values, 1, 10); expect(resetPage.page).toBe(1); expect(resetPage.items[0]).toBe('ID-1'); expect(resetPage.totalPages).toBe(6); const ui = fs.readFileSync(path.resolve('src/main.tsx'), 'utf8'); expect(ui).toContain('function ExpandableList'); expect(ui).not.toContain('story.relations.slice(0, 60)'); });
