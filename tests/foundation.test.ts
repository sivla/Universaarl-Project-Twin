...
const bcBasicContract = { path: 'exports/project-data/v1/index.yaml', expectedProjectId: 'UABC-BC-BASIC-001' } as const;
...
describe('Normalisierungsvertrag', () => {
  ...
  it('zeigt bekannte Statuswerte deutsch und unbekannte belegte Werte quelltreu', () => { expect(displayStatus('archived')).toBe('Archiviert'); expect(displayStatus('W0 Complete')).toBe('Nicht unterstützt (Quellstatus: W0 Complete)'); expect(displayStatus('')).toBe('Nicht belegt'); });
...
describe('Adapter- und Sicherheitsgrenzen', () => {
  ...
  it('adressiert als Bildnachweis ausschließlich reguläre PNG-Blobs unter evidence', async () => {
    ...
    expect(state.evidenceItems.some((item) => item.id === foreignId)).toBe(false); expect(resolveEvidenceId(root, foreignId)).toBeNull(); expect(await dispatchProjectApi('GET', `/api/projects/universaarl/evidence/${foreignId}`, productionRegistry(root))).toEqual({ status: 404, body: { code: 'NACHWEIS_NICHT_GEFUNDEN' } });
  ...
  it('redigiert absolute Hostpfade und URI-artige Quellwerte vor State und API', async () => {
    ...
    const api = await dispatchProjectApi('GET', '/api/projects/universaarl/state', productionRegistry(root)); const apiBody = JSON.stringify(api.body); expect(api.status).toBe(200); for (const leaked of [root, tenantGuid, nGuid, encodedNGuid, encodedGuid, doubleEncodedNGuid, 'privateCustomerAcme', 'customer-acme', doublePrivate, doubleQuery, encodedFragment, 'http://', '%25']) expect(apiBody).not.toContain(leaked); expect(apiBody).toContain(safeLearn); expect(apiBody).toContain(safeOpenSpec);
  });
  it('redigiert narrative Kennungen und sensible Wertpaare teilinhaltserhaltend in allen gerenderten Quellfamilien', async () => {
    ...
    expect(api.status).toBe(200); expect(serialized).toContain('[Kennung redigiert]'); expect(serialized).toContain('[Wert redigiert]'); expect(serialized).toContain(embeddedIdentifier); expect(serialized).toContain(commitLike);
  }, 20_000);
  ...
  it('liefert Evidence nach Worktree-Austausch oder Loeschung weiterhin nur als Commitbytes', async () => {
    ...
    fs.writeFileSync(file, Buffer.from('Worktree-Manipulation')); expect(resolveEvidenceId(root, id)?.bytes).toEqual(validPng); expect((await dispatchProjectApi('GET', `/api/projects/universaarl/evidence/${id}`, productionRegistry(root))).binary?.bytes).toEqual(validPng);
    fs.unlinkSync(file); expect(resolveEvidenceId(root, id)?.bytes).toEqual(validPng); expect((await dispatchProjectApi('GET', `/api/projects/universaarl/evidence/${id}`, productionRegistry(root))).binary?.bytes).toEqual(validPng);
  }, 15_000);
...
describe('indexgebundener project-data/v1-Vertrag', () => {
  it('liest ausschließlich indexierte BC-Basic-Quellen und bewahrt fehlende Einzelwerte als null', async () => {
    const root = projectDataFixture();
    const state = await createBoundTwinState('bc-basic', root, { projectDataContract: bcBasicContract });
  ...
  it('wendet Projektvertrag und Nachweisgrenze auch an der API an', async () => {
    const root = projectDataFixture(); const registry = productionRegistry(root);
    const response = await dispatchProjectApi('GET', '/api/projects/bc-basic/state', registry);
    expect(response).toMatchObject({ status: 200, body: { source: { projectId: 'bc-basic' }, evidenceItems: [] } });
    const commit = git(root, ['rev-parse', 'HEAD']); const oid = git(root, ['rev-parse', 'HEAD:evidence/unreferenziert.png']);
    const unindexedId = `ev_${createHash('sha256').update(`bc-basic\0${commit}\0${oid}\0evidence/unreferenziert.png`).digest('hex').slice(0, 24)}`;
    expect(await dispatchProjectApi('GET', `/api/projects/bc-basic/evidence/${unindexedId}`, registry)).toEqual({ status: 404, body: { code: 'NACHWEIS_NICHT_GEFUNDEN' } });
  });

  it('scheitert bei fehlendem Index, falscher Projekt-ID und fehlender Pflichtquelle geschlossen', async () => {
    const cases = [
  ...
  it('lehnt Pfadüberschreitung, Symlink, falschen Modus und verschärfte Größenüberschreitung ab', async () => {
    const traversal = projectDataFixture(); const indexFile = path.join(traversal, 'exports/project-data/v1/index.yaml'); fs.writeFileSync(indexFile, fs.readFileSync(indexFile, 'utf8').replace('atlassian/jira/issues/bc-basic.yaml', '../bc-basic.yaml')); commitAll(traversal, 'pfadüberschreitung');
    await expect(createBoundTwinState('bc-basic', traversal, { projectDataContract: bcBasicContract })).rejects.toBeInstanceOf(AdapterSourceError);

    const symlink = projectDataFixture(); const target = git(symlink, ['hash-object', '-w', '--stdin'], 'evidence/verification-register.yaml'); git(symlink, ['update-index', '--add', '--cacheinfo', `120000,${target},evidence/verification-register.yaml`]); git(symlink, ['commit', '-m', 'symlink']);
    await expect(createBoundTwinState('bc-basic', symlink, { projectDataContract: bcBasicContract })).rejects.toBeInstanceOf(AdapterSourceError);

    const executable = projectDataFixture(); git(executable, ['update-index', '--chmod=+x', 'atlassian/jira/issues/bc-basic.yaml']); git(executable, ['commit', '-m', 'falscher modus']);
    await expect(createBoundTwinState('bc-basic', executable, { projectDataContract: bcBasicContract })).rejects.toBeInstanceOf(AdapterSourceError);

    const oversized = projectDataFixture();
    await expect(createBoundTwinState('bc-basic', oversized, { projectDataContract: bcBasicContract, limits: { maxTextBytes: 64 } })).rejects.toBeInstanceOf(AdapterSourceError);
  }, 20_000);
});

describe('projektgescopte API', () => {
  it('liefert die normale Projektliste vollständig und explizit projektgebundenen Universaarl-State', async () => { const root = fixture(); const registry = productionRegistry(root); const list = await dispatchProjectApi('GET', '/api/projects', registry); const expected = [{ id: 'universaarl', key: 'UABC', name: 'Universaarl' }, { id: 'bc-basic', key: 'BCB', name: 'Business Central Basic' }]; expect(list).toMatchObject({ status: 200, body: { projects: expected } }); expect(projectListFromApiBody(list.body)).toEqual(expected); const result = await dispatchProjectApi('GET', '/api/projects/universaarl/state', registry); expect(result).toMatchObject({ status: 200, body: { source: { projectId: 'universaarl' } } }); expect(JSON.stringify(result.body)).not.toContain(root); });
  ...
  it('liefert ausschließlich feste deutsche Fehlercodes und verbirgt Rohfehler vollständig', async () => { const reader = vi.fn().mockRejectedValue(new Error('Fetch failed: C:\\Users\\secret\\tenant.json')); const unavailable = await dispatchProjectApi('GET', '/api/projects/universaarl/state', productionRegistry('C:\\missing'), reader); expect(unavailable).toEqual({ status: 503, body: { code: 'QUELLE_NICHT_VERFUEGBAR' } }); const missing = await dispatchProjectApi('GET', '/api/project-state', productionRegistry('C:\\x')); expect(missing).toEqual({ status: 404, body: { code: 'ENDPUNKT_NICHT_GEFUNDEN' } }); expect(await dispatchProjectApi('POST', '/api/projects', productionRegistry('C:\\x'))).toEqual({ status: 405, body: { code: 'METHODE_NICHT_ERLAUBT' } }); expect(JSON.stringify([unavailable, missing])).not.toMatch(/Fetch|Users|secret|tenant|error|message/i); expect(apiErrorCodes).toEqual(['METHODE_NICHT_ERLAUBT', 'ENDPUNKT_NICHT_GEFUNDEN', 'ANFRAGE_UNGUELTIG', 'PROJEKT_NICHT_GEFUNDEN', 'PROJEKTLISTE_ZU_GROSS', 'QUELLE_NICHT_VERFUEGBAR', 'NACHWEIS_NICHT_GEFUNDEN', 'API_NICHT_VERFUEGBAR']); expect(fs.readFileSync(path.resolve('vite.config.ts'), 'utf8')).toContain('{"code":"API_NICHT_VERFUEGBAR"}'); });
  it('bindet identische Commitblobs an die Projekt-ID und macht A-IDs unter B unauflösbar', async () => { const root = fixture(); const registry = createProjectRegistry([{ id: 'projekt-a', key: 'PRJA', name: 'Projekt A', sourceRoot: root }, { id: 'projekt-b', key: 'PRJB', name: 'Projekt B', sourceRoot: root }]); const stateA = (await dispatchProjectApi('GET', '/api/projects/projekt-a/state', registry)).body as ProjectState; const stateB = (await dispatchProjectApi('GET', '/api/projects/projekt-b/state', registry)).body as ProjectState; expect(stateA.source.commit).toBe(stateB.source.commit); expect(stateA.source.projectId).toBe('projekt-a'); expect(stateB.source.projectId).toBe('projekt-b'); expect(stateA.evidenceItems[0].id).not.toBe(stateB.evidenceItems[0].id); expect((await dispatchProjectApi('GET', `/api/projects/projekt-a/evidence/${stateA.evidenceItems[0].id}`, registry)).binary?.bytes).toEqual(validPng); expect(await dispatchProjectApi('GET', `/api/projects/projekt-b/evidence/${stateA.evidenceItems[0].id}`, registry)).toEqual({ status: 404, body: { code: 'NACHWEIS_NICHT_GEFUNDEN' } }); expect(await dispatchProjectApi('GET', `/api/projects/projekt-a/evidence/${stateB.evidenceItems[0].id}`, registry)).toEqual({ status: 404, body: { code: 'NACHWEIS_NICHT_GEFUNDEN' } }); }, 20_000);
  it('isoliert Nachweise projektgebunden und lehnt Traversal-IDs ab', async () => { const root = fixture(); const registry = productionRegistry(root); const state = await createTwinState(root); expect((await dispatchProjectApi('GET', `/api/projects/universaarl/evidence/${state.evidenceItems[0].id}`, registry)).binary?.bytes).toEqual(validPng); expect(await dispatchProjectApi('GET', '/api/projects/universaarl/evidence/..%2Fsecret', registry)).toEqual({ status: 404, body: { code: 'NACHWEIS_NICHT_GEFUNDEN' } }); expect(await dispatchProjectApi('GET', `/api/projects/fremd/evidence/${state.evidenceItems[0].id}`, registry)).toEqual({ status: 404, body: { code: 'PROJEKT_NICHT_GEFUNDEN' } }); });
});
