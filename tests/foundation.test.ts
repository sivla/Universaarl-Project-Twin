...
describe('Normalisierungsvertrag', () => {
  ...
  it('liefert vollständige Listen und klemmt angeforderte Seiten deterministisch', () => { const values = Array.from({ length: 57 }, (_, index) => `ID-${index + 1}`); const result = boundedList(values, 24); expect(result.items).toHaveLength(57); expect(result.items).toEqual(values); expect(result.visible).toBe(57); expect(result.total).toBe(57); expect(result.limited).toBe(false); expect(result.disclosureThreshold).toBe(24); expect(new Set(result.items).size).toBe(57); const filtered = values.filter((value) => value.endsWith('1') || value.endsWith('2')); const filteredPage = paginateList(filtered, 99, 10); expect(filteredPage.page).toBe(filteredPage.totalPages); expect(filteredPage.total).toBe(filtered.length); const firstPage = paginateList(values, 1, 10); expect(firstPage.page).toBe(1); expect(firstPage.items[0]).toBe('ID-1'); expect(firstPage.totalPages).toBe(6); const ui = fs.readFileSync(path.resolve('src/main.tsx'), 'utf8'); expect(ui).toContain('function ExpandableList'); expect(ui).not.toContain('story.relations.slice(0, 60)'); });
