...
describe('producerdefinierter Präsentationsvertrag', () => {
  it('validiert drei Wissensräume, zwei Jira-Ansichten und vier Tickettypen gemeinsam', () => {
    ...
    expect(presentationFixture.jira.ticketTypes.map((item) => item.type)).toEqual(['phase', 'epic', 'story', 'task']);
    ...
    expect(presentationFixtureState.story?.tickets).toHaveLength(18);
    expect(presentationFixtureState.presentation?.jira.canonicalTicketCount).toBe(18);
    expect(presentationFixtureState.story?.controls).toMatchObject({ worklogHours: 80, worklogCost: 9600 });
  });

  it.each<PresentationFixtureVariant>(['cycle', 'duplicate-id', 'duplicate-order', 'unknown-reference', 'invalid-initial-state', 'unknown-icon', 'invalid-parent', 'nonbillable-task', 'rollup-mismatch', 'phase-ref-mismatch', 'phase-container', 'wrong-phase-order', 'phase-billable', 'epic-without-phase', 'duplicate-epic-reference'])(
