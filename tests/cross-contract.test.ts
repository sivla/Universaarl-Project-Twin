...
describe.runIf(enabled)('commitgebundener Twin-Blueprint-Vertrag', () => {
  it('bindet den offiziell validierten Branch-Commit samt Katalog positiv und falsche Anker fail-closed', async () => {
    ...
    try {
      ...
      expect(state.setupWave).toMatchObject({ readOnly: true, writesAuthorized: false, target: { pilotName: 'Universaarl GmbH (BC Basic Pilot)', legacyName: 'Universaarl GmbH (Legacy)' }, configurationState: { baselineKind: 'standard-cronus-demo', pilotConfigured: false, writesApplied: false, readbackStatus: 'pending', internalCompanyId: null, targetDecision: 'pending-wave-0-evidence', resetDecision: 'pending-resetpoint-evidence' }, preflight: { wave0Status: 'required-not-executed', resetPoint: { status: 'missing-blocker' } }, writeGate: { noGoSteps: ['RUN-06', 'RUN-07', 'RUN-08', 'RUN-09', 'RUN-10', 'RUN-11', 'RUN-12', 'RUN-13', 'RUN-14', 'RUN-15', 'RUN-16', 'RUN-17', 'RUN-18', 'RUN-19', 'RUN-20', 'RUN-21', 'RUN-22'] }, packages: [{ tables: 0, records: 0, errors: 0 }, { tables: 0, records: 0, errors: 0 }, { tables: 0, records: 0, errors: 0 }] });
