...
describe.runIf(enabled)('commitgebundener Twin-Blueprint-Vertrag', () => {
  it('bindet den offiziell validierten Branch-Commit samt Katalog positiv und falsche Anker fail-closed', async () => {
    ...
    try {
      ...
      expect(state.story?.tickets.reduce<Record<string, number>>((counts, ticket) => ({ ...counts, [ticket.status]: (counts[ticket.status] ?? 0) + 1 }), {})).toEqual({ 'in-progress': 1, blocked: 3, created: 46 });
      expect(state.story?.offer).toMatchObject({ currentVersion: 'pilot-rebaseline-2026-07-13', versions: [], plannedHours: 80, plannedCost: 9600, actualHours: 0.25, actualCost: 30 });
      expect(state.story?.controls).toMatchObject({ worklogHours: 0.25, worklogCost: 30, realBcExecution: false });
      expect(state.story?.tickets.flatMap((ticket) => ticket.worklogs)).toHaveLength(1);
      ...
      expect(rawStory).toMatchObject({ classification: 'current-pilot-planning', status: 'in-progress', historicalClassification: 'archived-in-git-history', activeOffer: { status: 'planned-not-accepted', plannedHours: 80, actualHours: 0.25, customerAcceptanceClaimed: false }, businessCentralPilotState: { baselineKind: 'standard-cronus-demo', pilotConfigured: false, writesApplied: false, readbackStatus: 'pending' } });
      ...
      expect(state.story?.tickets.find((ticket) => ticket.id === 'UABC-39')).toMatchObject({ status: 'blocked', estimateHours: 5, actualHours: 0.25, remainingHours: 4.75, billable: true, closedAt: null, comments: [{ id: 'COM-UABC-39-W0-01-START' }, { id: 'COM-UABC-39-W0-01-BLOCKED' }], worklogs: [{ hours: 0.25, cost: 30 }] });
