...
describe.runIf(enabled)('commitgebundener Twin-Blueprint-Vertrag', () => {
  it('bindet den offiziell validierten Branch-Commit samt Katalog positiv und falsche Anker fail-closed', async () => {
    ...
    try {
      ...
      expect(state.story?.offer).toMatchObject({ currentVersion: 'pilot-rebaseline-2026-07-13', versions: [], plannedHours: 80, plannedCost: 9600, actualHours: 2.25, actualCost: 270 });
      expect(state.story?.controls).toMatchObject({ worklogHours: 2.25, worklogCost: 270, realBcExecution: false });
      expect(state.story?.tickets.flatMap((ticket) => ticket.worklogs)).toHaveLength(2);
