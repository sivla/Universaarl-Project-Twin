...
describe.runIf(enabled)('commitgebundener Twin-Blueprint-Vertrag', () => {
  it('bindet den offiziell validierten Branch-Commit samt Katalog positiv und falsche Anker fail-closed', async () => {
    ...
    try {
      ...
      expect(state.story?.tickets.reduce<Record<string, number>>((counts, ticket) => ({ ...counts, [ticket.status]: (counts[ticket.status] ?? 0) + 1 }), {})).toEqual({ 'in-progress': 1, blocked: 2, created: 47 });
      expect(state.story?.offer).toMatchObject({ currentVersion: 'pilot-rebaseline-2026-07-13', versions: [], plannedHours: 80, plannedCost: 9600, actualHours: 0, actualCost: 0 });
      expect(state.story?.controls).toMatchObject({ worklogHours: 0, worklogCost: 0, realBcExecution: false });
      expect(state.story?.tickets.flatMap((ticket) => ticket.worklogs)).toHaveLength(0);
      ...
      expect(rawStory).toMatchObject({ classification: 'current-pilot-planning', status: 'in-progress', historicalClassification: 'archived-in-git-history', activeOffer: { status: 'planned-not-accepted', plannedHours: 80, actualHours: 0, customerAcceptanceClaimed: false }, businessCentralPilotState: { baselineKind: 'standard-cronus-demo', pilotConfigured: false, writesApplied: false, readbackStatus: 'pending' } });
      ...
      expect(state.setupWave).toMatchObject({ readOnly: true, writesAuthorized: false, target: { pilotName: 'Universaarl GmbH (BC Basic Pilot)', legacyName: 'Universaarl GmbH (Legacy)' }, configurationState: { baselineKind: 'standard-cronus-demo', baselineProvenance: 'microsoft-standard-cronus-demo-data', pilotConfigured: false, writesApplied: false, readbackStatus: 'pending', internalCompanyId: null, targetDecision: 'pending-wave-0-evidence', resetDecision: 'pending-resetpoint-evidence', targetState: { classification: 'bc-basic-target-not-applied' }, appliedDifference: { status: 'none-evidenced', readbackEvidenceCount: 0 }, companyStrategyGate: { status: 'blocked-pending-wave0-and-reset-evidence', selectedOption: null, decisionEvidenceCount: 0, nextExecutableStep: 'W0-01-read-company-identity', writesAuthorized: false } }, preflight: { wave0Status: 'required-not-executed', resetPoint: { status: 'missing-blocker' } }, writeGate: { nextAllowedStep: 'W0-01-read-company-identity', noGoSteps: ['RUN-06', 'RUN-07', 'RUN-08', 'RUN-09', 'RUN-10', 'RUN-11', 'RUN-12', 'RUN-13', 'RUN-14', 'RUN-15', 'RUN-16', 'RUN-17', 'RUN-18', 'RUN-19', 'RUN-20', 'RUN-21', 'RUN-22'] }, packages: [{ tables: 0, records: 0, errors: 0 }, { tables: 0, records: 0, errors: 0 }, { tables: 0, records: 0, errors: 0 }] });
